...
router = APIRouter(prefix="/api/pei", tags=["PEI e Inclusão"])
...
async def create_pei(
    ...
    db.query(PEI).filter(
        PEI.aluno_id == pei_data.aluno_id,
        PEI.ativo == 1
    ).update({"ativo": 0})
...
async def get_pei_historico(
    ...
    peis = db.query(PEI).filter(PEI.aluno_id == aluno_id).order_by(PEI.data_inicio.desc()).all()
    return peis
...
@router.post("/{pei_id}/intervencoes", response_model=IntervencaoPedagogicaResponse, status_code=status.HTTP_201_CREATED)
...
@router.get("/{pei_id}/intervencoes", response_model=List[IntervencaoPedagogicaResponse])
async def list_intervencoes(
    ...
    intervencoes = db.query(IntervencaoPedagogica).filter(
        IntervencaoPedagogica.pei_id == pei_id
    ).order_by(IntervencaoPedagogica.data_intervencao.desc()).all()
